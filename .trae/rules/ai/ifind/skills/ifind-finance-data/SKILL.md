...
## 首次使用
- **配置密钥**: 插件安装后通过 connector 注入 `IFIND_AUTH_TOKEN`；本地独立使用时可在 `mcp_config.json` 写入 `IFIND_AUTH_TOKEN`。如不存在有效密钥，需提示用户到 "MCP官网 -> 个人中心 -> 密钥" 路径获取，帮助其完成密钥配置
...
1. 调用脚本需要可读取有效密钥：统一使用 `IFIND_AUTH_TOKEN`，可来自 connector 注入或 `mcp_config.json`

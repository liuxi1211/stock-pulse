...
| **AKQuant** | 0.2.47 | 量化计算（指标计算/回测），其 `akquant.talib` 与 `talib` API 对齐 |
...
> ⚠️ `services/indicator/`、`services/backtest/` 为废案已清空，待基于「统一策略配置 Schema」+ akquant 知识库重写。`services/factor/`（标准因子库）**已实现**。

| 模块 | 状态 | 重写依据 |
|---|---|---|
| 因子计算（`services/factor/`） | ✅ **已实现** | 见下方「标准因子库」；走 `akquant.talib`，`compute(name, inputs, **params)` 签名 |
| 技术指标（`services/indicator/`） | ⏳ 待重写 | akquant 知识库 07-talib-indicators；**禁止 pandas 自行实现** |
| 策略回测（`services/backtest/`） | ⏳ 待重写 | 统一 Schema 字段 → `aq.run_backtest(...)` 映射；akquant 知识库 04-backtest-run / 08-recipes |
...
为多因子选股、策略回测、条件表达式引擎提供统一的因子元数据管理（CRUD）+ 实时计算能力。
设计依据：[`../.trae/specs/002-standard-factor-library/spec.md`](../.trae/specs/002-standard-factor-library/spec.md)。

### 四层架构

```
定义加载（registry）→ Provider 层 → Service 层（calculator）→ API 层
```

| 层 | 文件 | 职责 |
|---|---|---|
| API 层 | `api/v1/factor.py` | CRUD + 计算 HTTP 接口（prefix `/python/v1/factors`） |
| Service 层 | `services/factor/calculator.py` | Provider 路由 / 参数校验 / 批量计算 / output_index 降维 |
| Provider 层 | `services/factor/providers/` | `AkquantTalibProvider` / `RawDataProvider` / `DerivedProvider` |
| 定义加载 | `services/factor/registry.py` | JSON 加载 / 原子持久化 / CRUD / 线程锁 / `get_lookback` |
| 数据工具 | `services/factor/data_utils.py` | watcher OHLCV `list[dict]` → `dict[str, ndarray]` |

### 因子规模

共 **54** 个标准因子（设计稿 `标准因子库-v2.json` 含 55，按用户决策删除 akquant 不支持的 `STOCHRSI`）：
- **AKQUANT（31）**：技术面，100% 走 `akquant.talib`（MA/MACD/KDJ/BOLL/RSI/ATR/…）。KDJ 由 `STOCH` 合成 `J=3K−2D`。
- **RAW（5）**：OPEN/HIGH/LOW/CLOSE/VOLUME 直通。
- **DERIVED（2）**：VOL_MA/VOL_EMA（MA/EMA 作用于 volume）。
- **TUSHARE（16）**：基本面，仅注册元数据，计算返回 `400 FACTOR_NOT_COMPUTABLE`（由 watcher 取数）。

### 运行时文件（不触库）

- 种子（入库）：`data/factors.default.json` —— 缺失/损坏时据此恢复。
- 运行时（.gitignore）：`data/factors.json` —— CRUD 写入此处，原子写入（临时文件 + `os.replace`）。

### 主要接口

| 接口 | 方法 | 说明 |
|------|------|------|
| `/python/v1/factors/categories` | GET | 因子分类 |
| `/python/v1/factors` | GET | 因子列表（category? / source?） |
| `/python/v1/factors/{factorKey}` | GET | 因子详情 |
| `/python/v1/factors` | POST | 新增因子（201） |
| `/python/v1/factors/{factorKey}` | PUT / DELETE | 修改 / 删除 |
| `/python/v1/factors/compute` | POST | 单标的多因子计算 |
| `/python/v1/factors/batch-compute` | POST | 多标的批量计算 |

错误响应统一含 `errorCode`：`UNKNOWN_FACTOR` / `FACTOR_NOT_COMPUTABLE` / `INVALID_PARAM` / `INVALID_OUTPUT_INDEX` / `FACTOR_NOT_FOUND` / `FACTOR_ALREADY_EXISTS`。

### 测试

```bash
pytest tests/test_factor -v          # 44 用例（registry/providers/calculator/api/no-db）
python tests/test_factor/benchmark.py  # NFR-1 性能基准
```

watcher 侧通过 `FactorService`（Caffeine 缓存 + 写操作失效）访问本服务，前端页面 `/factor-library`。
...
- **Conda 环境**：`stock`
- **Python**：3.12+
- **依赖**：akquant 0.2.47 / fastapi 0.115.7 / uvicorn 0.41.0 / pandas 3.0.3 / numpy 2.4.5 / pydantic 2.13.4 / loguru 0.7.3
...
- **健康检查**：访问 http://127.0.0.1:8085/health
...
> 标准因子库接口（`/python/v1/factors/**`）已实现（见上）。技术指标 `/quote/*`、策略回测 `/backtest/*` 待重写。基础端点：

| 接口 | 方法 | 说明 |
|------|------|------|
| `/` | GET | 服务状态检查 |
| `/health` | GET | 健康检查 |
| `/python/v1/factors/**` | GET/POST/PUT/DELETE | 标准因子库（元数据 CRUD + 计算） |

...
public class ExpressServiceImpl implements ExpressService, DataCheckable {
     ...
     * <p>
     * 遍历 [startDate, endDate] 区间内每个自然日，调用 {@link TushareClient#expressByAnnDate(String)}
     * 拉取该公告日全市场快报记录；空结果跳过，命中即按 A 类事务模式（TransactionTemplate 包裹 saveBatch）落库。
     * <p>
     * 入口定位：与 {@link #fetchAndSaveAllByRange} 互为补充--后者按 ts_code 逐只拉取（兼容历史 dispatch），
     * 本方法按 ann_date 全市场拉取（增量同步推荐入口；快报月度约 86 行，远低于 5000 行截断阈值）。
     *
    ...
    public int fetchAndSaveByAnnDateRange(String startDate, String endDate) {
        ...
        if (start.isAfter(end)) {
            log.warn("express ann_date 增量拉取：startDate={} 晚于 endDate={}，跳过", startDate, endDate);
            return 0;
        }
        log.info("按 ann_date 增量拉取 express [{}~{}]", startDate, endDate);
        int total = 0;
        int hitDays = 0;
        for (LocalDate d = start; !d.isAfter(end); d = d.plusDays(1)) {
            String annDate = d.format(DATE_FMT);
            List<ExpressDTO> rows;
            try {
                // ⚠️ API 调用在事务外执行，避免限流等待时长时间占用数据库连接
                rows = tushareClient.expressByAnnDate(annDate);
            } catch (Exception e) {
                log.warn("express ann_date={} 拉取失败: {}", annDate, e.getMessage());
                continue;
            }
            if (rows == null || rows.isEmpty()) {
                continue;
            ...
            total += entities.size();
            log.info("express ann_date={} 保存 {} 条", annDate, entities.size());
        }
        log.info("express ann_date 增量拉取完成，命中 {} 个公告日，共 {} 条记录", hitDays, total);
        return total;
    }

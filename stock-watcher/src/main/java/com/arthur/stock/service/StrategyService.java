...
/**
 * 策略管理服务（spec 004 Task 6）。
 * <p>
 * 所有方法以 <b>uuid（TEXT 业务 ID，仅用于前端交互）</b> 为入参，内部先查主表 quant_strategy 拿 INTEGER PK（id），
 * 再操作 quant_strategy_version。版本快照表的 strategyId 字段存的是主表 PK（Long），不是 uuid。
 * <p>
 * 关键约束：
 * <ul>
...
public interface StrategyService {
     ...
     * 策略详情，含当前版本 config。
     */
    StrategyDTO getStrategyDetail(String uuid);

    /**
     * 更新策略元信息（不改 config）。
     */
    void updateStrategy(String uuid, StrategyUpdateRequest req);

    /**
     * 更新策略配置（生成新版本）。含长度校验、JSON 校验、乐观锁、engine 校验、状态机自动转换。
     */
    StrategyDTO updateStrategyConfig(String uuid, StrategyConfigUpdateRequest req);
     ...
     */
    void updateStatus(String uuid, StrategyStatusUpdateRequest req);

    /**
     * 版本列表（不含 configJson）。
     */
    List<StrategyVersionDTO> listVersions(String uuid);

    /**
     * 单个版本详情（含 configJson）。
     */
    StrategyVersionDTO getVersion(String uuid, Integer versionNo);

    /**
     * 版本对比。对 fromVer / toVer 的 configJson 做 JSON Diff。
     */
    List<StrategyDiffDTO> diffVersions(String uuid, Integer fromVer, Integer toVer);

    /**
     * 回滚到指定版本。复用 updateStrategyConfig 链路：以目标版本 config 写新版本，
     * 重新走 engine 校验、乐观锁与状态机。
     */
    StrategyVersionDTO rollbackVersion(String uuid, StrategyRollbackRequest req);
     ...
     * @param toVer   目标版本号
     */
    com.arthur.stock.dto.strategy.StrategyVersionCompareDTO compareVersions(String uuid, Integer fromVer, Integer toVer);
}

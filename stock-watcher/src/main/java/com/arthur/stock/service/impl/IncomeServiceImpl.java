...
public class IncomeServiceImpl implements IncomeService, DataCheckable {
    ...
    public int fetchAndSaveIncome(String tsCode, String startDate, String endDate) {
        ...
        List<IncomeDO> entities = rows.stream()
                .map(this::toEntity)
    ...
    public int fetchAndSaveAllByRange(String startDate, String endDate) {
        ...
        List<CompletableFuture<Integer>> futures = stocks.stream()
                .map(s -> CompletableFuture.supplyAsync(() -> {
                    try {
                        ...
                        return fetchAndSaveIncome(s.getTsCode(), startDate, endDate);
        ...
        log.info("income fetchAndSaveAllByRange 区间 {}~{} 并发拉取完成, 共 {} 条", startDate, endDate, total);
        return total;
    }

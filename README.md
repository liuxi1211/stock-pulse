...
> 详细设计文档见 `sdlc/design/002~005/`，原型见 `sdlc/prd/002~005/prototype/`
...
│
├── stock-engine/                      # Python 计算服务层（端口 8000）
...
| 因子库 | `sdlc/prd/002-因子库/` · `sdlc/design/002-因子库/` | 因子分类体系与注册机制 |
| 策略管理 | `sdlc/prd/003-策略管理/` · `sdlc/design/003-策略管理/` | 规则树 JSON Schema、版本管理 |
| 回测中心 | `sdlc/prd/004-回测中心/` · `sdlc/design/004-回测中心/` | 单次/网格/WF 回测设计 |
| 选股模块 | `sdlc/prd/005-选股模块/` · `sdlc/design/005-选股模块/` | 规则树→SQL 翻译引擎 |
| K 线与复权 | `sdlc/design/001-K线计算与复权方案/` | K 线计算与复权公式 |

> 每个模块的 `sdlc/prd/xxx/prototype/` 下含可点击浏览的 HTML 原型；`sdlc/design/xxx/` 下含主设计、DB 设计、schema.sql、HTTP API 四份文档。
